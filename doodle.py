...
class Paint:
    ...
    def color_bucket(self):
        color_box = messagebox.askokcancel("FILL COLOR", "This is applicable only for the available shapes."
                            " Select the color from the chooser with which color you want inside the shape"
                             " and then draw the shape you wish")
   ...
   # ZOOM USING BUTTONS
    ...
    def copy(self, _=None):  # Copy the selected region
        try:
            ...
            messagebox.showwarning("Error","Please select the region first")
    ...
    def paste(self, _=None):  # Paste the region keep in clipboard
        if (self.counter >= 0):
            ...
            messagebox.showwarning("Error","Cut or Copy the region to Paste ")
    ...
    def show_frame(self):
        if self.mode == "Gesture mode":
            ...
            lmlist, bbox = detector.findPosition(self.img)
            ...
            if len(lmlist) != 0:
                ...
                print(fingers)
                cv2.rectangle(self.img, (framexR, frameyR), (wCam - framexR, hCam - frameyR), (255, 0, 255), 2)

                # 4. Only Index Finger : Moving Mode
                if fingers[1] == 1 and fingers[2] == 0 and fingers[3] == 0 and fingers[4] == 0 and fingers[0] == 0:
                # 5. Convert Coordinates
                    x3 = np.interp(x1, (framexR, wCam - framexR), (0, wCanvas))
                    x3 = np.interp(x1, (framexR, wCam - framexR), (0, wCanvas))
                    y3 = np.interp(y1, (frameyR, hCam - frameyR), (0, hScr))
                    # 6. Smoothen Values
                    self.clocX = self.plocX + (x3 - self.plocX) / smoothening
                    self.clocY = self.plocY + (y3 - self.plocY) / smoothening
                    # 7. Move Mouse
                    autopy.mouse.move(wCanvas - self.clocX, self.clocY)
                    cv2.circle(self.img, (x1, y1), 5, (255, 0, 255), cv2.FILLED)
                ...
                if fingers[1] == 1 and fingers[2] == 1:
                    ...
                    length, self.img, lineInfo = detector.findDistance(8, 12, self.img)
                    #print(length)
                    
                    # 10. Click mouse if distance short
                    if length < 20:
                        cv2.circle(self.img, (lineInfo[4], lineInfo[5]), 5, (0, 255, 0), cv2.FILLED)
            ...
            if _:
                cv2image = cv2.cvtColor(self.img, cv2.COLOR_BGR2RGB)
                img1 = PIL.Image.fromarray(cv2image)
